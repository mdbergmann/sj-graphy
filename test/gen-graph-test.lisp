(defpackage :sj-graphy.gen-graph-test
  (:use :cl :fiveam :sj-graphy.gen-graph)
  (:export #:run!
           #:all-tests
           #:nil))
(in-package :sj-graphy.gen-graph-test)

(def-suite gen-graph-tests
  :description "Tests for generating package graphs"
  :in sj-graphy.tests:test-suite)

(in-suite gen-graph-tests)

(test gen-graph--flat-packages
...
(test gen-graph--flat-packages-with-dependencies
...
(test gen-graph--flat-packages-with-dependencies--exclude-edge
...
(test gen-graph--flat-packages-replace-node-names
...
(test gen-graph--clustered-with-dependencies
  (let* ((node-sets `((:name "fooc"
                       :cluster-attributes (:bgcolor :green)
                       :nodes (,(pak-to-node (spak:make-pak :name "bar"))))
                      (:name "barc"
...
(test gen-graph--clustered-with-edges-between-clusters
